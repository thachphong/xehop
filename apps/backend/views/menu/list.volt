                    ...
                    <table id="datatable-responsive" class="table table-striped table-bordered dt-responsive nowrap" cellspacing="0" width="100%">
                      ...
                      {%for item in menus%}
                        <tr>                          
                          ...
                          <td>{{item['type']}}</td>
                          <td>{%if item['page_flg'] =='4'%}{{item['link']}}{%else%}{{item['dm_name']}}{%endif%}</td>                                           
                          <td>                            
...
<script>
  $(document).ready(function() {
    ...
    $(document).off('click','#btn_new'); 
      ...
      Pho_json_ajax('POST',"{{url.get('menu/update')}}" ,arr,function(datas){
        if(datas.status =="OK"){
          Pho_modal_close("modal1");
        ...
        Pho_json_ajax('GET',"{{url.get('menu/delete/')}}" + id,null ,function(datas){
          if(datas.status == "OK"){
            //Pho_modal_close("modal1");
            //Pho_message_box("Thông báo",datas.msg);
        ...
        $(document).on('click','.btn_edit',function(event){
          var id = $(this).attr('id');
            id = id.replace("edit_","");      
              Pho_html_ajax('GET',"{{url.get('menu/edit/')}}"+ id ,null,function(html){
                /*Pho_modal({
                template:html,
                closeClick: false,
                closable: true, 
        ...
        $(document).on('click','.add_child',function(event){
          var id = $(this).attr('id');
            id = id.replace("add_","");     
            Pho_html_ajax('GET',"{{url.get('menu/addchild/')}}"+ id ,null,function(html){
                /*Pho_modal({
                template:html,
                closeClick: false,
                closable: true, 
